...
import { BRAND_CONFIGS, getBrandLabel } from '@/app/_lib/brand'
import type { BrandId } from '@/app/_lib/brand'
...
const formatSeconds = (v: number | null) => v == null || isNaN(v) ? '—' : `${(v / 1000).toFixed(1)} s`
const formatScore = (v: number | null | undefined) => v == null || isNaN(v) ? '—' : `${v}%`
...
function BrandBar() {
  return (
    <header className="brand-bar">
      <img className="logo-left" src="/Assets/vi-logo.png" alt="VI movies & tv" />
...
function BrandFilterBar({ brandFilter, onBrandChange }: { brandFilter: BrandId; onBrandChange: (brand: BrandId) => void }) {
  return (
    <div className="brand-filter-bar">
      <span className="brand-filter-label">Dashboard:</span>
      <button
        type="button"
        className={`brand-filter-btn ${brandFilter === 'all' ? 'is-active' : ''}`}
        onClick={() => onBrandChange('all')}
      >
        All Brands
      </button>
      {BRAND_CONFIGS.map((config) => (
        <button
          key={config.brand}
          type="button"
          className={`brand-filter-btn ${brandFilter === config.brand ? 'is-active' : ''}`}
          style={brandFilter === config.brand ? { borderColor: config.color, background: `${config.color}15`, color: config.color } : {}}
          onClick={() => onBrandChange(config.brand)}
        >
          {config.label}
        </button>
      ))}
...
function Hero({ rows, lastUpdated, bestPageLoad, worstPageLoad, avgPageLoad, brandLabel }: { rows: any[], lastUpdated: string; bestPageLoad: number | null; worstPageLoad: number | null; avgPageLoad: number | null; brandLabel?: string }) {
  return (
      ...
      <div className="hero-left">
        ...
        <div>
          ...
          <div className="hero-sub">
            <span><IconGlobe /></span><span>Environment: {brandLabel ? `${brandLabel} Audit` : 'Browsing History Audit'}</span>
...
export default function Home() {
  ...
  const [brandFilter, setBrandFilter] = useState<BrandId>('all')

  const fetchWebsites = useCallback(async (opts?: { silent?: boolean }) => {
    if (!opts?.silent) setLoading(true)
    try {
      const params = brandFilter !== 'all' ? `?brand=${brandFilter}` : ''
  ...
  useEffect(() => {
    try {
      ...
      if (saved === 'vi' || saved === 'redbull' || saved === 'all') {
        setBrandFilter(saved)
      }
    } catch {}
  }, [])
  ...
  const filteredRows = useMemo(() => {
    if (brandFilter === 'all') return reportRows
    return reportRows.filter((row) => {
      try {
        const hostname = new URL(row.url).hostname.toLowerCase()
        const config = BRAND_CONFIGS.find((c) => c.brand === brandFilter)
        if (!config) return false
        return hostname === config.hostSuffix || hostname.endsWith(`.${config.hostSuffix}`)
      } catch {
        return false
      }
    })
  }, [reportRows, brandFilter])
  ...
  const comparisonRows = filteredRows.filter((row) => comparisonSelection.includes(row.url))

  // Calculate page load statistics from page-load time (TTI)
  const pageLoads = filteredRows
    .map((row) => row.metric?.tti)
    .filter((value): value is number => typeof value === 'number')

  const bestPageLoad = pageLoads.length ? Math.min(...pageLoads) : null
  const worstPageLoad = pageLoads.length ? Math.max(...pageLoads) : null
  const avgPageLoad = pageLoads.length ? pageLoads.reduce((sum, value) => sum + value, 0) / pageLoads.length : null

  const totalSites = filteredRows.length
  const latestAudit = filteredRows[0]?.audit
  ...
  if (loading) {
    ...
    reportContent = <p className="text-center text-slate-500 py-12">No audit data available yet for {getBrandLabel(brandFilter)}.</p>
  } else {
    reportContent = (
        ...
        <table className="min-w-full border-separate border-spacing-0 text-left text-sm text-slate-700">
          <thead>
            <tr className="bg-slate-950 text-slate-100">
              ...
              <th className="sticky top-0 border-b border-slate-800 bg-slate-950 px-4 py-3">URL - Prod - {getBrandLabel(brandFilter)}</th>
          ...
          <tbody>
            {filteredRows.map((row, index) => (
              <tr key={row.id} className={index % 2 === 0 ? 'bg-slate-50' : 'bg-white'}>
                ...
                <td className="border-b border-slate-200 px-4 py-4">{fmtCls(row.metric?.cls ?? null)}</td>
  ...
  const handleBrandChange = useCallback((brand: BrandId) => {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-4 py-10">
        <BrandFilterBar brandFilter={brandFilter} onBrandChange={handleBrandChange} />
        <section className="rounded-[32px] border border-slate-800 bg-slate-900/95 p-8 shadow-2xl shadow-slate-950/40">
          <div className="flex flex-col gap-8 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl text-white">
          {brandFilter === 'all' ? 'Lighthouse Performance Tracker' : getBrandLabel(brandFilter)}
        </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300">
                {brandFilter === 'all' ? 'Audit results aggregated across all brands.' : `Lighthouse performance audits for ${getBrandLabel(brandFilter)}.`}
              </p>
            </div>
        ...
        <div className="mt-10 grid gap-6 xl:grid-cols-[1.6fr_0.9fr]">
          <section className="rounded-3xl bg-white p-6 shadow-sm shadow-slate-900/5">
            <div className="flex items-center justify-between gap-4 pb-4 border-b border-slate-200/70">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">Run a Lighthouse audit</h2>
                <p className="mt-2 text-sm text-slate-500">
                  Enter any supported URL to save the latest Lighthouse report.
                </p>
              </div>
            </div>
            <div className="mt-6">
          ...
          <section className="rounded-3xl bg-slate-900 p-6 text-slate-100 shadow-sm shadow-slate-950/20">
            <h2 className="text-xl font-semibold text-white">{getBrandLabel(brandFilter)} — audit summary</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-3xl bg-slate-950/90 p-4 ring-1 ring-slate-800">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Brand filter</p>
                <p className="mt-3 text-2xl font-semibold text-white">{getBrandLabel(brandFilter)}</p>
              </div>
            </div>
          </section>
        </div>
        ...
        <section className="mt-10 rounded-3xl bg-white p-6 shadow-sm shadow-slate-900/5">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">{getBrandLabel(brandFilter)} — page performance</h2>
              <p className="mt-2 text-sm text-slate-500">
                Recent audited pages with Lighthouse metrics and load performance.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full border border-slate-400/20 bg-slate-100/5 px-3 py-1 text-sm font-medium text-slate-300">{getBrandLabel(brandFilter)}</span>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <>
          <Hero rows={filteredRows} lastUpdated={lastUpdated} bestPageLoad={bestPageLoad} worstPageLoad={worstPageLoad} avgPageLoad={avgPageLoad} brandLabel={getBrandLabel(brandFilter)} />
          {hoveredInsight ? <InsightPreview insight={hoveredInsight} /> : null}
          <AIAssistantPanel rows={filteredRows} />
          {showComparison && (
          ...
          <ReportTable rows={filteredRows} showHistory={showHistory} onPreviewInsight={handlePreviewInsight} onOpenInsight={handleOpenInsight} />
          <InsightDrawer
          ...
          <div className="bottom-panels">
            <PerformanceOverview rows={filteredRows} />
            <ScoreDistribution rows={filteredRows} />
            <PerformanceRating rows={filteredRows} />
          </div>
        </>
      )}
  </main>
